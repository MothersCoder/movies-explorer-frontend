...
function SearchForm (props) {
  ...
  const searchList = () => {
    return currentUrl.pathname === "/saved-movies"
      ? props.likedCards
    ...
    <section className="search-form">
      <form className="search-form__form">
        <input
          ...
          name="searchWords"
          value={formInputs.values.searchWords}
          onChange={formInputs.handleChange}
          placeholder="Фильм"
        />
        <input className="search-form__submit" onClick={handleClick}/>
      </form>
      <p className="search-form__err">
        {searchList() === undefined
          ? "По вашему запросу ничего не найдено :( Попробуйте уточнить запрос и повторить поиск"
          : props.queryError
          ? "Введите название фильма, ну или хотя бы его фрагмент"
          : ""}
      </p>
    </section>
  );
