...
function MoviesCardList (props) {
  ...
  const filmList = props.checkedStatus
    ? checkData(props.sortMovies)
    : props.movies;
  ...
  function getInitCount (windowWidth) {
    return windowWidth >= "1280"
      ? setFilmQuantityLoad(12)
      : windowWidth > "480"
      ? setFilmQuantityLoad(8)
      : windowWidth >= "320" &&  windowWidth <= "480"
      ? setFilmQuantityLoad(5)
      : setFilmQuantityLoad(5);
  }
  ...
  function hide () {
    return filmList === null || filmList === undefined
      ? "card-list__more-button_hide"
      : filmList.length <= filmQuantityLoad
      ? "card-list__more-button_hide"
      : "";
  }
    ...
    <section className="card-list">
      ...
      <ul
        className={`card-list__list ${
          currentUrl.pathname === "/saved-movies" ? "card-list__list_saved" : ""
        }`}
      >
        {render()}
      </ul>
      <button
        className={`card-list__more-button ${hide()} ${
          currentUrl.pathname === "/saved-movies" ? "card-list__more-button_hide" : ""
        }`}
