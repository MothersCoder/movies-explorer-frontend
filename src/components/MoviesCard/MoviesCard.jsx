...
function MoviesCard (props) {
  ...
  function likeCheck () {
    if(props.likedCards) {
      let likedMove = props.likedCards.filter(
        (movie) => movie.nameRU === props.data.nameRU
      );
  ...
  return (
    <li className="card-list__card" key={props.data.id || props.data._id}>
      ...
      {currentUrl.pathname === "/saved-movies" ? (
        ...
        <button
          type="button"
          className={`card-list__card-like ${
            isLiked ? 'card-list__card-like_active' : ''
          }`}
          onClick={onLikeClick}
          ></button>
      )}

      <p className="card-list__card-duration">
        {hour}ч {minute}м
      </p>
    </li>
  );
